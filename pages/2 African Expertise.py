...
def load_data():
    ...
    return pd.read_csv('afriq_df1.csv'), pd.read_csv('afriq_df2.csv'), pd.read_csv('afriq_df3.csv'), pd.read_csv('afriq_df4.csv'), pd.read_csv('afriq_df5.csv'), pd.read_csv('afriq_df6.csv')
...
if st.button('Regenerate Questions'):
    ...
    for filtered_df in selected_data:
        ...
        if len(filtered_true) > 5:
            filtered_true = filtered_true.sample(n=2,random_state=num)
        if len(filtered_false) > 5:
            filtered_false = filtered_false.sample(n=8,random_state=num)
    ...
    new_cols = ['sample_id', 'question', 'answer','rationale', 'model number', 'model_answer', 'model rationale']  
